...
for data in result:
    ...
    if len(bms_id) >= 1:
        ...
        while period_start < period_now:
            ...
            if max_temperature:
                ...
                period_value = period_end.strftime('%Y-%m-%d %H')
    ...
    for i in range(data_count):
        ...
        connect.hmset(f'{time}_{patient_number}_voltage_day_{i+1}', result_voltage[i])

        connect.expire(f'{time}_{patient_number}_temperature_day_{i+1}', 86400)
        connect.expire(f'{time}_{patient_number}_voltage_day_{i+1}', 86400)
