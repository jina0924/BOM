...
for data in result:
    ...
    for i in range(data_count):
        ...
        connect.hmset(f'{time}_{patient_number}_temperature_month_{i+1}', result_temperature[i])
        connect.hmset(f'{time}_{patient_number}_bpm_month_{i+1}', result_bpm[i])
        connect.hmset(f'{time}_{patient_number}_oxygen_saturation_month_{i+1}', result_oxygen_saturation[i])

        connect.expire(f'{time}_{patient_number}_temperature_month_{i+1}', 86400)
        connect.expire(f'{time}_{patient_number}_bpm_month_{i+1}', 86400)
        connect.expire(f'{time}_{patient_number}_oxygen_saturation_month_{i+1}', 86400)
